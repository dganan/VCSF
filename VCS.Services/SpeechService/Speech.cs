...
namespace VCS
{
	...
	{
		public static byte[] Speak(string text, Gender gender, Age age, Language language)
		{
			...
			{ 
				language = LanguageGesser.IdentifyLanguage(text);
			}
			...
			for (int i = 0; i < voices.Count && selectedVoice < 0; i++)
			{
				...
				if ((language == Language.English && voices.Item(i).GetAttribute("Language") == "809")
						||
						(language == Language.Spanish && voices.Item(i).GetAttribute("Language") == "C0A")
						||
						(language == Language.Catalan && voices.Item(i).GetAttribute("Language") == "403")
						||
						(language == Language.Italian && voices.Item(i).GetAttribute("Language") == "410"))
				{
					selectedVoice = i;
				}
			}
			...
			using (MemoryStream memoryStream = new MemoryStream())
			{
				...
				//using (FileStream fs = new FileStream(@"C:\VCS\TEMP\"+num+".wav", FileMode.Create, FileAccess.Write))
				//{
				//    num++;

				//    BinaryWriter writer2 = new BinaryWriter(fs);

				//    writer2.Write(memoryStream.ToArray());
